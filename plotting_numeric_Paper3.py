...
def plot_filter_effect(raw_data_green_list, filtered_data_green_list, raw_data_blue_list, filtered_data_blue_list):

    X_green_raw, Y_green_raw, Z_green_raw = zip(*raw_data_green_list)
    X_green_filtered, Y_green_filtered, Z_green_filtered = zip(*filtered_data_green_list)
    X_blue_raw, Y_blue_raw, Z_blue_raw = zip(*raw_data_blue_list)
    X_blue_filtered, Y_blue_filtered, Z_blue_filtered = zip(*filtered_data_blue_list)
    
    fig, axs = plt.subplots(3,1, figsize=(10, 8), sharex=True)

    axs[0].plot(X_green_raw, label="Unfiltered X green", linestyle="dotted", color="lime")
    axs[0].plot(X_green_filtered, label="Filtered X green", color="green")
    axs[0].plot(X_blue_raw, label="Unfiltered X blue", linestyle="dotted", color="cyan")
    axs[0].plot(X_blue_filtered, label="Filtered X blue", color="blue")
    axs[0].set_ylabel("X position [mm]")
    axs[0].legend()

    axs[1].plot(Y_green_raw, label="Unfiltered Y green", linestyle="dotted", color="lime")
    axs[1].plot(Y_green_filtered, label="Filtered Y green", color="green")
    axs[1].plot(Y_blue_raw, label="Unfiltered Y blue", linestyle="dotted", color="cyan")
    axs[1].plot(Y_blue_filtered, label="Filtered Y blue", color="blue")
    axs[1].set_ylabel("Y position [mm]")
    axs[1].legend()

    axs[2].plot(Z_green_raw, label="Unfiltered Z green", linestyle="dotted", color="lime")
    axs[2].plot(Z_green_filtered, label="Filtered Z green", color="green")
    axs[1].plot(Z_blue_raw, label="Unfiltered Y blue", linestyle="dotted", color="cyan")
    axs[1].plot(Z_blue_filtered, label="Filtered Z blue", color="blue")
    axs[2].set_ylabel("Z position [mm]")
    axs[2].set_xlabel("Frame")
    axs[2].legend()

    plt.suptitle("3D Kalman Filter for position tracking")
    plt.show()
